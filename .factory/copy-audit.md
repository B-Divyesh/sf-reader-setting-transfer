...
Audited 2026-08-29. Counts treat hyphenated terms as one word. Navigation,
field labels, numbered markers, and button labels are interface names. Every
sentence is at or below 22 words, and no banned term appears.
...
| 16 | For low-vision readers who want the same text size, spacing, contrast, and motion across public articles. |
...
| 11 | The extension applies the same text and contrast settings to each article. |
...
| 7 | Apply one reading card to every article. |
| 12 | Your saved reading card sets the article view and lists each active value. |
...
| 13 | Export your reading card as a text file and import it in another browser. |
...
| 8 | No account, tracking, remote fonts, or server upload. |
| 10 | Remove the extension and its local data leaves with it. |
| 9 | Install the extension, then open it on each article. |
...
| Distraction-reduced article surface | simplified article / reader |
| Per-site enable or disable choice | site choice |
| Temporary sample experience | demo |
| File used to move settings | reading card file (JSON in technical detail) |
